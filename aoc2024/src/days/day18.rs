...
impl Pos {
	...
	fn steps(&self, bytes: &HashSet<Pos>) -> Vec<(Pos, u32)> {
		...
		vec![(1, 0), (0, 1), (-1, 0), (0, -1)]
			.iter()
			.filter_map(|&(dx, dy)| match (x.checked_add(dx), y.checked_add(dy)) {
				(Some(x), Some(y)) if (0..=SIZE).contains(&x) && (0..=SIZE).contains(&y) => {
					match bytes.contains(&Pos(x, y)) {
...
impl DaySolver for Day {
	...
	fn two(&self, input: &str) -> DayResult {
		...
		for line in input.lines() {
			...
			match result {
				Some(_) => continue,
				None => return DayResult::success(format!("{},{}", a, b)),
			}
		}
