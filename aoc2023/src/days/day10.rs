...
fn get_inputs(input: &str) -> (Position, PipeMap) {
	let mut animal = (0, 0);
...
impl DaySolver for Day {
	fn one(&self, input: &str) -> DayResult {
		let (animal, map) = get_inputs(input);
		...
		loop {
			let PipeTile::Pipe(a, b) = current_tile.1 else {
				panic!()
			};
			let next_move = if a.reverse() == last_move { b } else { a };
			last_move = next_move;
			let pos = (
				current_tile.0 .0 + next_move.0,
				current_tile.0 .1 + next_move.1,
			);
	...
	fn two(&self, input: &str) -> DayResult {
		...
		let mut checked: Vec<Vec<bool>> = map
			.iter()
