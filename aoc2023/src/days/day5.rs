...
fn map_seed_range(seeds: Range<u64>, maps: &Vec<Vec<Map>>) -> u64 {
	...
	for map in maps {
		for range in ranges.iter_mut() {
			for map_range in map {
				...
				range.start = i.end;
				new_ranges
					.push(map_range.map(i.start).unwrap()..map_range.map(i.end - 1).unwrap() + 1);
			}
			if !range.is_empty() {
	...
	ranges.iter().map(|r| r.start).min().unwrap_or(u64::MAX)
}
	...
	for line in lines {
		if line.ends_with("map:") {
			if maps.len() > 0 {
				maps.last_mut()
					.unwrap()
					.sort_by(|a, b| a.src.start.cmp(&b.src.start));
			}
			maps.push(Vec::new())
...
impl DaySolver for Day {
	fn one(&self, input: &str) -> DayResult {
		...
		DayResult::success(seeds.iter().map(|s| map_seed(*s, &maps)).min().unwrap())
	}
