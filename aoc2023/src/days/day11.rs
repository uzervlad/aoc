...
fn get_inputs(input: &str) -> (Vec<(usize, usize)>, HashSet<usize>, HashSet<usize>) {
	...
	let empty_rows: HashSet<_> = (0..h)
		.filter(|y| bytes[y * w..(y + 1) * w].iter().all(|s| s == &b'.'))
		.collect();
	let empty_cols: HashSet<_> = (0..w)
		.filter(|x| (0..h).all(|y| bytes[y * w + x] == b'.'))
		.collect();

	let galaxies: Vec<_> = bytes
		.iter()
		.enumerate()
		.filter(|(_, s)| **s == b'#')
		.map(|(i, _)| (i % w, i / w))
